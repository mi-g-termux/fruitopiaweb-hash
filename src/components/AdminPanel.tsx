...
`Are you sure you want to permanently remove the subscriber"${item.email}" from your email marketing database index?`,
 ...
 className="w-full bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-800"
 /> <p className="text-[9px] text-slate-400 mt-1">Get your Client ID and Secret from <span className="font-semibold">developer.paypal.com → My Apps &amp; Credentials</span></p> </div> )}
 ...
 className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 rounded-lg px-2.5 py-1.5 text-xs font-semibold focus:outline-none"
  ...
  <div key={label} className="grid grid-cols-1 gap-2 bg-white border border-slate-100 rounded-lg p-2.5"> <div> <label className="block text-[9px] font-extrabold text-slate-500 uppercase mb-0.5">{label} — Logo Image URL <span className="normal-case font-normal text-violet-500">(recommended: 240 × 80 px, transparent PNG/SVG)</span></label> <div className="flex items-center gap-2"> <input
  type="text"
  value={logoVal}
  ...
  onError={(e) => { (e.target as HTMLImageElement).style.display ='none'; }}
  /> </div> )}
  </div> <p className="text-[9px] text-slate-400 mt-1">Preview shows the exact aspect ratio used on the checkout payment button.</p> </div> </div> ))}
