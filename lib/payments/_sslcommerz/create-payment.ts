...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Honor the sandbox toggle from the request body whenever it is provided,
    // independent of whether credentials came from the body or env vars.
    ...
    if (missing.length) {
      return res.status(400).json({ error: `Missing SSLCommerz credentials: ${missing.join(', ')}. Set Store ID and Store Password in the admin panel.` });
    }
