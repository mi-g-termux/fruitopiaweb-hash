...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Prefer admin-panel credentials sent from the client; fall back to env vars.
    let creds = await getGatewayCreds('nagad');
    const merchantIdFromBody = String(body.merchantId || '');
    const merchantNumberFromBody = String(body.merchantNumber || '');
    const privateKeyFromBody = String(body.merchantPrivateKey || body.privateKey || '');
    if (merchantIdFromBody) creds = { ...creds, merchantId: merchantIdFromBody };
    if (merchantNumberFromBody) creds = { ...creds, merchantNumber: merchantNumberFromBody };
    if (privateKeyFromBody) creds = { ...creds, privateKey: privateKeyFromBody };
