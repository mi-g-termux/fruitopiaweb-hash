...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Prefer admin-panel credential sent from the client; fall back to env var.
    let creds = await getGatewayCreds('stripe');
    const secretFromBody = String(body.stripeSecretKey || body.secretKey || '');
    if (secretFromBody) {
      creds = { ...creds, secretKey: secretFromBody };
    }

    const missing = missingCreds(creds, ['secretKey']);
    if (missing.length) {
      return res.status(400).json({ error: `Missing Stripe credentials: ${missing.join(', ')}. Set them in the admin panel.` });
    }
