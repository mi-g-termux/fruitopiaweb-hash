...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    let creds = await getGatewayCreds('razorpay');
    const keyIdFromBody = String(body.keyId || '');
    const keySecretFromBody = String(body.keySecret || '');
    if (keyIdFromBody && keySecretFromBody) {
      creds = { ...creds, keyId: keyIdFromBody, keySecret: keySecretFromBody };
    }

    const missing = missingCreds(creds, ['keyId', 'keySecret']);
    if (missing.length) {
      return res.status(400).json({ error: `Missing Razorpay credentials: ${missing.join(', ')}. Set them in the admin panel.` });
    }
