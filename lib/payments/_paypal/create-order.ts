...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    let creds = await getGatewayCreds('paypal');
    const clientIdFromBody = String(body.clientId || '');
    const clientSecretFromBody = String(body.clientSecret || '');
    if (clientIdFromBody && clientSecretFromBody) {
      creds = { ...creds, clientId: clientIdFromBody, clientSecret: clientSecretFromBody };
    }
    ...
    if (missing.length) {
      return res.status(400).json({ error: `Missing PayPal credentials: ${missing.join(', ')}. Set them in the admin panel.` });
    }
